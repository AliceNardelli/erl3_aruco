...
* The robot explores goes at the centre of each room using move base. Than it rotates, then for ten times in goes to a random target generated with move base and rotates. When it has reach a target and it has found an hint it stops for querying the ontology.
...
In that case the publisher subscriber communication protocol is used for perception and execution. In particular the reactive navigation use laser data an the aruco detection use the camera data.
...
* In that case the robot randomly reaches 10 poses for room. It doesn't metter if find all the aruco of the room, after 10 motions it changes room. A possible improvement is to make perceives all the 5 aruco before leaving it. Since the navigation modules give me a lots of troubles
...
* A possible optmization of the Gmapping algorithm consists in sobstituting the **Gmapping (FastSLAM)** with the **KartoSLAM** which is optimal since it use a graph search approach and computes directly the edges and verteces for navigation.
